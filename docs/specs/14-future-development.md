...
### GPU Utilization Metrics (Observability Gap)

**Problem (found 2026-07-06):** The embedding/reranker path runs a single GPU-backed container (`embeddings-gpu`, Infinity, serving both the dense embedder and the `bge-reranker-v2-m3` cross-encoder from the same process — see `docs/deployment/embeddings.md`) with `deploy.resources.reservations.devices: count: all` in `docker-compose.yml` and no server-side batch-size/concurrency limit (only the client-side `embedding.remote_batch_size: 32` in `config.yaml`). Whether this setup is leaving GPU throughput on the table or is already near-saturated is currently unknown, because the stack has **no GPU utilization signal at all**: no DCGM/`nvidia-smi` exporter job in `docker/prometheus/prometheus.yml`, and none of the existing Grafana dashboards (system, qdrant, memexd, graph, lsp, mcp, etc. under `docker/grafana/dashboards/`) covers GPU.

**Approach:** Add an `nvidia-smi`-based or DCGM Prometheus exporter as a new compose service, gated behind the existing `embeddings-gpu` profile so it only runs when a GPU is actually present, scraped by the existing Prometheus. Add a new `docker/grafana/dashboards/gpu.json` tracking at minimum: GPU utilization %, VRAM used/free, and (if the exporter surfaces it) a per-process breakdown so the embedder-vs-reranker VRAM/compute split is visible.

**Why it matters:** Any decision to raise Infinity's batch size or concurrency to use more of the GPU is currently a guess. This closes the measurement gap first; throughput tuning (batch size, concurrent workers) is a follow-up once utilization is actually observed, not before.

### Branch-not-indexed vs. pattern-absent — empty-result disambiguation

**Finding (2026-07-06, usage feedback):** The single real trap in a heavy `grep`/`search` session was that project-scoped reads default to the caller's *current git branch* (`resolveEffectiveBranch`, `tools/branch-scope.ts`). On a freshly-created, not-yet-indexed feature branch, exact/regex patterns that unquestionably exist on `main` (`@Service` in a Spring project, `.now()`, `printStackTrace`) returned 0 — **indistinguishable from "the pattern does not exist"**. It cost several calls to diagnose (the tell was "`@Service` = 0 in a Spring repo is impossible → wrong branch"). Workaround: pass `branch:"main"` explicitly (the user persisted this as a `grep-branch` behavioral rule).

**Why the existing mitigation is incomplete (VERIFIED 2026-07-06):** `grep` already auto-widens to all branches on an empty scoped result (`widenGrepToAllBranches`, #151) and returns a message. Ruled OUT by the reporter's precise repro + a deployment probe: it was `grep` not semantic `search` (so not a widen-less path); cwd/tenant resolved (`branch:"main"` and other branches returned the 145 `@Service` normally); and the running container was confirmed to CONTAIN #151 (`branch-scope.js` + `widenGrepToAllBranches` present in `/app/.../dist`, built Jul 6 17:08) — so it is NOT a stale deploy. The failing branch was created via `git checkout -b` seconds earlier and had **zero indexed files**. Conclusion: the auto-widen genuinely fails to fire (or fails to surface) for a branch that **exists in git but has an empty index** (distinct from an unresolved tenant or an empty project). The widen's own guard — "returns `undefined` when the all-branches requery is also empty" — is the suspect; something in the empty-index-branch path skips the widen or short-circuits before the all-branches requery runs. Net UX: a bare "No matches" that conflates *absent* with *branch-not-indexed*. (The semantic-`search` auto-widen gap noted here was CLOSED 2026-07-07 — see the Approach note below.)

**Approach (IMPLEMENTED for `grep` 2026-07-06):** Make "no indexed content on the requested branch" a first-class, proactive signal instead of relying only on the post-hoc widen. `GrepTool.diagnoseEmptyGrep` (`src/typescript/mcp-server/src/tools/grep.ts`) now runs on a still-empty project-scoped result: when the requested concrete branch has zero indexed files (via `SearchDbReader.listBranchCounts`), it returns `branchNotIndexedMessage` ("branch `X` has no indexed content yet — indexed branches: {main, …}; pass `branch:"*"` …") instead of a bare "No matches". Best-effort (swallows reader errors), fires only on the empty path. **Parity extended (IMPLEMENTED 2026-07-07):** the diagnosis was extracted to a shared `tools/empty-diagnosis.ts` (`diagnoseEmptyResult` + `branchNotIndexedMessage`/`pathFilterExcludedAllMessage`) and wired into **exact `search`** (`search-exact.ts`, via a threaded `SearchDbReader`); and **semantic `search`** gained branch auto-widen on empty (`search.ts` re-runs `runSearchAndFinalize` with `branch:"*"`, REUSING the embeddings — no re-embed — and returns the widened hits with a widen note). So grep/exact/semantic now all widen + diagnose. **Still open:** the underlying widen-doesn't-fire-on-empty-index-branch question (§ above) — the shared diagnosis now papers over it with a clear message, but a direct daemon-side reproduce of why #151's widen didn't surface is still worth doing.

**Second silent-zero mode — pathGlob filters to empty (reported 2026-07-06, separate from branch):** With `branch:"main"` explicit, a rooted-partial `pathGlob` (`management/test/**/*.dart`, `**/AdvertisingGrpcService.java`) returned 0 while the content existed — confirmed by re-running WITHOUT the glob. Code trace (`text_search/escaping.rs`): globs match against the **absolute** `file_path`, and `normalize_path_glob` auto-prefixes `**/` to relative patterns so they float; both the FTS5 and grep-fallback paths run the same `resolve_path_filter` → matcher pipeline (no divergence). Two findings: (1) **multi-segment literal path components must be ADJACENT** — `management/test/` floats as a block and only matches when those two dirs are consecutive in the absolute path; a real path like `.../management/<x>/test/...` won't match (the likely `management/test/**` cause — glob semantics, arguably user error). (2) `**/AdvertisingGrpcService.java` *should* match a file of that name anywhere per the code, so if it truly returned 0 with the search term present in that file, that needs a concrete repro (exact search term + one absolute path) to confirm a real bug vs. the term simply not being in that file. **Unifying fix idea (IMPLEMENTED for `grep` 2026-07-06):** the same "distinguish absent from filtered" principle applies to the glob dimension. `GrepTool.diagnoseEmptyGrep` now, on a still-empty result carrying a `pathGlob`/`pathExclude`, re-runs the SAME branch scope with NO path filter; if that finds hits it returns `pathFilterExcludedAllMessage` ("No matches under pathGlob …, but N+ without the path filter — the path filter excluded everything; pathGlob matches the ABSOLUTE path and multi-segment literals must be ADJACENT; a name-only filter should float as `**/Name.ext`"). This runs before the branch probe (path filter is the more specific cause) and only on the empty path. **Extended (IMPLEMENTED 2026-07-07):** the path-filter probe is now in the shared `diagnoseEmptyResult` and runs for **exact `search`** too. Separately, the **semantic `pathGlob` was made a real FLOATED glob** — its post-filter (`search-helpers.ts` `filterResultsByPathGlob`) used both-ends-anchored `matchesGlob`, so `management/test/**` only matched when the path STARTED with it; it now uses `matchesPathInclude` (floats like `pathExclude` and grep's daemon glob). So `pathGlob` semantics are consistent across grep/exact/list/semantic. **Still open:** the possible-real-bug on `**/AdvertisingGrpcService.java` still wants a concrete repro to confirm (the float fix may already resolve it).

**Related, lower-value gotchas from the same session (not necessarily backlog-worthy, recorded for context):**
- **Regex is RE2** (no look-around): `(?<!\.)` is rejected; `\b` is the workaround. **Cosmetic `%3F` bug — FIXED (2026-07-06):** gRPC transmits the status message percent-encoded and `@grpc/grpc-js` surfaces it undecoded, so `?`→`%3F` (and `\n`→`%0A`) reached the caller. Fixed centrally at the `grpcUnary` choke point (`clients/daemon-client/connection.ts` → `decodePercentEncodedGrpcMessage`) so every daemon-backed tool (grep/exact/list/retrieve/graph) now renders real text. **Follow-ups:** (a) **FIXED 2026-07-07** — the daemon mapped a *malformed user regex/glob* (`SearchDbError::InvalidPattern`) to `Status::internal`; now `map_search_error` (`text_search_service.rs`) returns `Status::invalid_argument` with an RE2-aware message for `InvalidPattern` (shared by `search` + `count_matches`), keeping `internal` for genuine DB/IO faults. (b) still open (optional): `exact-hints.ts` could pre-warn on look-around/backreference syntax before the round-trip.
- **Graph centrality/hotspots polluted by test fixtures** (PageRank dominated by `remove`/`key`/`toString` and test-only nodes). This is NOT a gap — the `WQM_GRAPH_CENTRALITY_EXCLUDE` env var already exists for exactly this. Caveat: it matches path *substrings*, not globs, so `test/,fixture,__tests__` works but `**/test/**` does not. Consider documenting it in the `graph` tool description / surfacing a hint, since it's easy to miss.

### MCP Tool Gaps vs. CLI (`wqm`) Surface

**Finding (2026-07-06):** The MCP server exposes 11 tools (`search`, `retrieve`, `rules`, `store`, `scratchpad`, `grep`, `list`, `embedding`, `graph`, `workspace_index`, `search_eval`), but the `wqm` CLI has a materially larger surface. An audit of `src/typescript/mcp-server/src/tool-definitions/*.ts` against the CLI/gRPC surface found the following NOT reachable from any MCP tool today:

- **Daemon health / indexing status.** No tool answers "is the daemon healthy, is indexing caught up" before an agent trusts a `search` result. `embedding` only reports the embedding-provider config, not overall health; `workspace_index`'s `indexing_status`/`observe_project` give aggregate queue progress but not process health or per-item failure diagnosis. Highest-value gap — it's a trust signal an agent currently has no way to check itself.
- **`wqm admin ignore-candidates`.** Ranks directories polluting the index by file-count/failure-rate/extension-homogeneity — directly relevant to the recurring "legacy/vendored dir polluting search" problem (see [[search-legacy-dir-noise-and-debug-fallback]] in project memory). No MCP equivalent exists.
- **`wqm queue stats/list/show`.** Per-item failure diagnosis (not just aggregate progress).
- **`wqm tags list/search/tree/stats`.** Inspecting extracted keyword/tag data — no MCP path at all, not even partial.
- **`wqm admin stats` / `token-savings` / `search-latency`.** Search instrumentation analytics — lower priority for an agent than the above.

**Deliberately excluded (not a gap to close):** `wqm admin backup/restore`, `reembed`, `rebuild`, `rename-tenant`, `cleanup-orphans` — destructive/admin state mutations. `docs/specs/08-api-reference.md`'s "Removed/Automated Features" table already establishes the precedent (daemon-owned collection lifecycle, diagnostic-not-agent-facing) for keeping this class of operation CLI/human-gated rather than agent-callable; that reasoning still applies here. `wqm project list/status/register` largely overlaps `workspace_index`'s `list_projects`/`project_status`/`add_project` already — not a real gap.

**When to revisit:** health/status is the strongest candidate to implement first (trust signal, read-only, no new risk surface). `ignore-candidates` as a read-only MCP tool is the second candidate. Tags and admin-stats are nice-to-haves, not blocking anything today.
...
**Why it fits:** `grep` (line-anchored FTS5) and the code graph (line-anchored nodes) already provide the precision the embedding index lacks — this is a retrieval-routing/reranking layer over existing capabilities, not new storage. It composes with the cross-encoder reranker already in the search path and the relevance-ranking design in [Cross-Project Search](#cross-project-search). Until built, the workaround is the manual recipe (semantic-locate → targeted `grep` on error/annotation/throw → read only the method), captured as the example-monorepo `debug-rule-line` behavioral rule.
