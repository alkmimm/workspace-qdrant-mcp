...
- **Credential redaction** (`document_processor/redaction.rs`): one policy, `redact_for_path`, applied at both places text leaves the disk — the chunker (`process_file_sync_inner`, feeding vectors + Qdrant payload) and the FTS5 line index, whose writers in `strategies/processing/file/` read the disk on their own and must use `redaction::read_for_index` (static check `scripts/ci/forbid_bare_index_reads.sh`; the first deploy shipped the chunker hook alone and `grep` served a token the vectors had masked). Configuration-shaped files (`.properties .conf .cfg .ini .cnf .toml .yaml .json .xml .plist .tf …`) get the key/value layer — a key ending in `password`, `secret`, `api_key`, `token`, `private_key`, … has its value replaced by `<redacted>`, quotes kept, placeholders (`${VAR}`, `{{ x }}`, booleans, numbers, `var.x`, calls) left alone; every file gets the known-token layer (AWS/GitHub/GitLab/Slack/OpenAI/Stripe/Google/SendGrid/npm/HF tokens, JWTs, `scheme://user:pass@` URLs, PEM private-key bodies). The file's metadata carries `redacted_lines`. This is why `.conf .properties .cfg .ini .cnf` are on the allowlist (measured: one in six carried credentials) and why `.env*` still is not — a file that is nothing but secrets is not worth a regex's residual risk; only `.env.example` / `.sample` / `.template` / `.dist` are admitted by name. Files indexed before this landed keep their old chunks until they change or a `make reembed` visits them (the chunker fingerprint was not bumped on purpose)
