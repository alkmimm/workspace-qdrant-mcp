...
- **Admin UI: paused state + state-aware Pause/Resume buttons** — the registered-projects table rendered the status pill purely from `isActive` (active/idle) and always showed both Pause and Resume buttons, so a paused watch was visually indistinguishable from a running one (the snapshot payload didn't even carry `is_paused`). The `/admin/api/snapshot` handler now merges `ListWatches` (which carries `is_paused`) into each registered project, adding `isPaused`; the status pill shows an amber `paused` badge when paused, and the row shows only the applicable control — Pause when running, Resume when already paused.
- **Per-request host CWD propagation (MCP server)** — new `utils/request-context.ts` module exposes an `AsyncLocalStorage`-backed `getEffectiveCwd()` that resolves the client's working directory from (1) the `X-MCP-Host-Cwd` HTTP header bound per request by `mcp-http-server.ts`, (2) the `WQM_DEFAULT_HOST_CWD` env var, or (3) `process.cwd()`. Every tool handler that previously called `process.cwd()` for project detection (`grep`, `list`, `retrieve`, `rules`, `search-exact`, `search-helpers`, session init) now goes through `getEffectiveCwd()`. Unblocks the dockerized HTTP transport, where the container's `process.cwd()` is fixed at `WORKDIR /app` and never matches any registered project; also unblocks stdio invocations launched from a directory outside a registered project when paired with the env-var fallback.
...
- **`is_active` session-counter leak — per-session tracking (schema v42)** — `watch_folders.is_active` (the priority/LSP gate) is meant to count live MCP sessions, but `register_session`/`unregister_session` mutated it as a free-running `+/- 1` counter with no record of which sessions were counted. Sessions that ended ungracefully — and the MCP server's own `ensureSelfRepoRegistered`, which re-registers the self-repo on every restart with no balancing decrement — leaked `+1` forever (observed: the self-repo at `is_active=12`). Replaced with a real per-session model: a new `project_sessions` table (one row per `(tenant, collection, session_id)`) from which `is_active` is projected as `COUNT(*)` of live sessions (`WatchFolderLifecycle::sync_is_active_from_sessions`), so register/unregister/heartbeat are idempotent and cannot leak. `session_id` was added to `RegisterProject`/`DeprioritizeProject`/`Heartbeat`; the MCP server threads `sessionState.sessionId`, and the self-repo uses the stable id `"self-repo"` (re-register = no-op) kept alive by a dedicated heartbeat. `cleanup_orphaned_sessions` reaps rows with no heartbeat within the timeout. The migration resets the leaked counters (self-repo 12 → 0). `set_priority`/`unregister_session_by_path` still use the legacy `+/- 1` counter (test-only / worktree path-scoped; self-heals on the next sync). Verified: clippy `--lib --bins --tests -D warnings` clean; 17 `priority_manager` + 3 `schema_version::v42` unit tests pass; migration applied live (41 → 42).
- **`list` MCP tool ignored its `branch` argument** — `buildListOptions` (`tool-builders/list.ts`) parsed every list argument except `branch`, so `branch:"*"` / `branch:"main"` were silently dropped and `list` always fell back to the caller's current Git branch, showing only the (sparsely-indexed) current branch (e.g. 56 of 2210 files on a feature branch). The `search` and `grep` builders already parsed it; `list`'s omission was the gap. Now parsed, restoring all branch selections. Verified live: `list branch:"*"` 42→2227, `branch:"main"` 42→2177. (Audit §8.1)
- **`list` showed only changed files on a feature branch** — the daemon tags only CHANGED files under a feature branch (unchanged files keep the project's base-branch tag), so a branch-scoped `list` on a feature branch missed the bulk of the project. `list` now merges in base-branch files that are unchanged on the current branch (the project as it appears on that branch) and suppresses the stale base copy of any file that IS changed (shown in its feature version instead). The base branch is resolved from the indexed data (`getBaseBranch` — the branch the daemon tagged the bulk under), not git's local default, which can differ (files tagged `main` while the git default is `master`). No-op on the base branch and for `branch:"*"`. Verified live: `list` default 56→2210. (Audit §8.2)
- **Project detection fails for Windows subpaths and worktrees (MCP server)** — `queryProjectByPath` built the longest-prefix `LIKE` pattern as ``path || '/' || '%'`` with a hardcoded `/`, so a Windows host CWD like ``C:\…\workspace-qdrant-mcp\.claude\worktrees\xyz`` never matched a watch-folder row stored as ``C:\…\workspace-qdrant-mcp``. Both sides of the comparison are now normalized via `replace(_, '\', '/')` (JS-side for the bind value, SQL-side for the stored column), so any mix of `\` and `/` resolves correctly. Restores `grep`, `list`, and project-scoped `search`/`retrieve` from inside parallel git worktrees on Windows.
