...
pub struct DaemonMetrics {
    ...
    /// Shared files whose content DIVERGES on a worktree's branch (git-Modified
    /// vs main) enqueued by the membership reconcile, read from the worktree tree.
    /// Labels: tenant_id, branch
    pub worktree_membership_divergent_enqueued_total: IntCounterVec,
...
struct CreatedMetrics {
    ...
    worktree_membership_divergent_enqueued_total: IntCounterVec,
...
fn create_all_metrics() -> CreatedMetrics {
    ...
    let worktree_membership_divergent_enqueued_total = int_counter_vec(
        "worktree_membership_divergent_enqueued_total",
        "Shared files divergent on the worktree branch (git-Modified vs main) enqueued from the worktree tree by the membership reconcile",
        &["tenant_id", "branch"],
    );
...
fn register_metrics(registry: &Registry, m: &CreatedMetrics) {
    register_all(
        ...
        vec![
            ...
            Box::new(m.worktree_membership_divergent_enqueued_total.clone()),
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            worktree_membership_new_on_branch_enqueued_total: m
                .worktree_membership_new_on_branch_enqueued_total,
            worktree_membership_divergent_enqueued_total: m
                .worktree_membership_divergent_enqueued_total,
