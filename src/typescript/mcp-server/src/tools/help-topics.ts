...
export const HELP_TOPICS = {
  ...
  http: {
    ...
    content: [
      ...
      'fallback — nothing about your call selected it), or "unresolved" (NO project was resolved at',
      'all). Anything but "cwd" or "projectId" means the project was INFERRED: if the echoed path is',
      'not the repo you meant, re-issue the call with `cwd` set. "unresolved" is stronger than that:',
      'the result is not a statement about any repository, so an empty list there is NOT evidence',
      'that the project is empty — pass `cwd` or `projectId` and re-run before concluding anything.',
    ].join(' '),
  },
  rules: {
