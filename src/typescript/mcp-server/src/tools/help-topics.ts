...
export const HELP_TOPICS = {
  ...
  scratchpad: {
    ...
    content: [
      ...
      'lane appends the most relevant notes after the code hits. Over HTTP, a cwd-less write with',
      'no session project silently files the note under the GLOBAL tenant, where project-scoped',
      'search never resurfaces it — so pass `cwd` (or `projectId`). To revise or remove a note,',
      'use the `scratchpad` tool (update/delete) rather than creating near-duplicates.',
    ].join(' '),
  },
  branches: {
  ...
  http: {
    ...
    content: [
      ...
      'server-side defaults. Read tools refuse with "Could not detect project" when nothing',
      'resolves; some write paths do not — a cwd-less scratchpad store silently lands in the',
      'GLOBAL tenant (see the scratchpad topic).',
    ].join(' '),
  },
  rules: {
