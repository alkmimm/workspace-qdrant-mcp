  ...
  inputSchema: {
    ...
    properties: {
      ...
      responseFormat: {
        ...
        description:
          'Response verbosity. Governs the BODY of each hit only — metadata is trimmed to the same small set in every format, so passing "concise" explicitly changes nothing (it is already the default; use "summary" or "packed" if you want fewer tokens). "concise" (default) truncates each hit body with a retrieve() marker; "detailed" returns full bodies (per-hit cap off); "packed" returns ONE ranked, deduplicated context bundle (packed_bundle.text: per-hit "location · symbol" header + capped body) assembled under maxResponseBytes, with metadata-only entries in results — the most token-efficient way to READ several hits at once; "summary" drops every body and returns metadata only — the strongest, most token-efficient format, identical to the summary:true boolean (either spelling works). Use the default for discovery, packed when you intend to read the top hits, summary to just locate files.',
      },
      maxResponseBytes: {
        type: 'number',
        description:
