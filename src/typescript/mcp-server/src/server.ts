    ...
    return new Server(
      { name: SERVER_NAME, version: SERVER_VERSION },
      {
        ...
        instructions: [
          ...
          'Query formulation — write `search` queries in ENGLISH regardless of the conversation language. The embedding model is multilingual, but code identifiers and comments are overwhelmingly English and cross-lingual recall for *code* is weak: a non-English query matches same-language prose/docs instead of the code and recall collapses. Use vocabulary close to the expected identifiers/comments. When you want the implementation rather than docs or tests, add fileType="code" (other values: text, config, data, docs, web, slides, build) or a pathGlob like "src/**/*.rs" — documentation and test files often outrank the implementation otherwise. Note: prose documentation and Markdown are classified "text"; fileType="docs" is for binary document formats (PDF, Office), so to bias toward project docs use "text", not "docs". When a legacy, vendored, or generated tree pollutes the results (e.g. a dead "old_project/"), pass `pathExclude` — a glob that is the hard opposite of `pathGlob` — on `search`, `grep`, or `list` to drop it (it floats, so "old_project/**" removes that dir at the repo root and any nested depth); a deployment may also demote such paths automatically in semantic ranking via WQM_SEARCH_DERANK, so results already sink without you asking.',
          ...
          'Project memory (scratchpad) — as you work, proactively record durable project knowledge with `store` type="scratchpad": decisions and their rationale, non-obvious gotchas, conventions, in-flight task state other agents/sessions may need, and anything worth recalling in a later session. Keep each note self-contained. Each write is stamped with provenance (origin_branch/origin_cwd/origin_worktree — pass `branch` explicitly when writing from a git worktree the server cannot see) so notes are attributable to the branch/worktree that produced them; provenance is attribution only, never a read filter. Notes are project-scoped and resurface AUTOMATICALLY — the project-scoped `search` recall lane appends the most relevant notes after the code hits, so you need not query the scratchpad explicitly. To revise or remove a note, use the `scratchpad` tool (update/delete, by point `id` from `scratchpad list` or by verbatim content) rather than creating near-duplicates.',
