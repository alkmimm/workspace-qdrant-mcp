    ...
    return new Server(
      { name: SERVER_NAME, version: SERVER_VERSION },
      {
        ...
        instructions: [
          ...
          'Exact lookups — use `grep` for regex / exact substring across the project (faster and cheaper than `search` with exact=true for known strings). Use `list` (start with format="summary") to understand layout before drilling in. Use `retrieve` when you already know the Qdrant point id from a `search`/`list` result or a metadata filter. The `documentId` argument must be the result `id` field, not `metadata.document_id`; if you only have the metadata hash, use `filter: { document_id: "..." }`. The tool will also try that metadata filter automatically when a direct point lookup misses. When `retrieve` returns `success:false`, read its `hint` before retrying.',
